        ...
        public void StartCondition()
        {
            foreach(var item in taskEventBase.GetTaskValues())
            {
                if (item.Key.isActorTypeNumber())
                {
                    ...
                    {
                        levelActor.AddEventMessagesToHandlerForward(new InitEventMessage(actorid:levelActor.GetCreateInternalComponentBase().GetCreateID(),actortype: item.Key, camp: LevelActorBase.EnemyCamp, point_x: EnemyPoint[i % EnemyPoint.Length].X, point_y: EnemyPoint[i % EnemyPoint.Length].Y, angle: 0));
